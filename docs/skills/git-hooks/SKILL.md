...
3. 仅在用户明确要求启用或更新 hooks 时执行本流程，且操作范围仅限 `.git/hooks/` 下的这三个文件。

## 创建步骤

用文件写入工具将下面三个模板**原样**写入对应路径（已存在同名 hook 时先向用户确认再覆盖），然后：
...
# merge、fixup 和 squash 等 git 自动生成的信息直接放行；revert 仍使用 `revert:`。
