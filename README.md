...
依赖使用规范见 [docs/dependency-management.md](docs/dependency-management.md)，完整目录设计见 [docs/project-structure.md](docs/project-structure.md)。
...
需要 Docker Desktop 4+（内含 Docker Compose v2）。Android 模拟器使用 `frontend/.env.example` 中的 `10.0.2.2` 访问宿主机 API；真机调试时，将 `EXPO_PUBLIC_API_URL` 改为开发机器的局域网 IP。
